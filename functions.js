...
// Declare the variable in the broader scope
let publicKey = null;

function fetchPublicKey() {
    return fetch('/raw/js/public.key') // Adjust path if necessary
        .then(response => {
            if (!response.ok) {
                throw new Error('Failed to fetch public key');
            }
            return response.text(); // Return the PEM string
        });
}
...
// Initialize the public key when the page loads or at a specific event
...
return  publicKeyPem;
        ...
        .then(key => {
            publicKey = key; // Set the publicKey variable
        })
        .catch(error => {
    ...
    loadCSS: function (cssURL, linkid, loadAfterTheme) {
        ...
        function sanitizeInput(input, type) {
            ...
            if (type === 'url') {
                ...
                // Basic ID validation: only alphanumeric characters and hyphens are allowed
        ...
        try {
            var sanitizedCssURL = sanitizeInput(cssURL, 'url');
            var sanitizedLinkid = sanitizeInput(linkid, 'id');
            ...
            if (doc.createStyleSheet) { // IE
                ...
                for (var i = 0; i < sheets.length; i++) {
                    var sh = sheets[i];
                    var h = sh.href;
                    if (h === sanitizedCssURL) {
                        cnt++;
                        if (sh.disabled) {
                            sh.disabled = false;
                            return;
                        } else {
                            if (o_info.debug) o_logwarn("BLoader::loadCSS: style: " + sanitizedCssURL + " already in document and not disabled! (duplicate add)");
                            return;
                        }
                    }
                ...
                doc.createStyleSheet(sanitizedCssURL, pos);
            } else { // Mozilla
                var el = jQuery('#' + sanitizedLinkid);
                if (el && el.length > 0) {
                    if (o_info.debug) o_logwarn("BLoader::loadCSS: stylesheet already found in doc when trying to add:" + sanitizedCssURL + ", with id " + sanitizedLinkid);
                } else {
                    var newSt = jQuery('<link>', {
                        id: sanitizedLinkid,
                        rel: 'stylesheet',
                        href: sanitizedCssURL
                    });
                    if (loadAfterTheme) {
            ...
            if (o_info.debug) {
                o_logerr('BLoader::loadCSS: Error when loading CSS from URL::' + sanitizedCssURL);
            }
        }
    },
...
//				var msgUploadLimit = trans.translate('file.upload.limit');
				...
				if(maxSize < 250 * 1024) {
					...
					maxSizeFormatted = Math.ceil(maxSize / 1024 / 1024).toFixed(0) + " MB";
				} else {
					maxSizeFormatted = Math.ceil(maxSize / 1024 / 1024 / 1024).toFixed(1) + " GB";
				}
				fileInputElement.setCustomValidity("Maximum " + maxSizeFormatted + " file size can be uploaded.");
			}
		}
	}
    ...
    var fileType = fileInputElement.files[0].type;
    if (!allowedTypes.includes(fileType)) {
        fileInputElement.setCustomValidity("Only JPEG and PNG formats are accepted.");
    }
			...
			jQuery.ajax(targetUrl, {
                ...
                success: function(returnedData, textStatus, jqXHR) {
                    ...
                    if (returnedData) {
                        ...
                        var sanitizedData = {
                            businessPath: o_escapeHtml(returnedData.businessPath),
                            documentTitle: o_escapeHtml(returnedData.documentTitle),
                            historyPointId: o_escapeHtml(returnedData.historyPointId)
                        };
...
function processData(data, publicKey) {
    return new Promise((resolve, reject) => {
        try {
            const data2 = {}; // Initialize an empty object to hold the processed data
            const formLength = data.length;

            // Track completion of asynchronous operations
            let pendingPromises = [];

            // Process each item in the data array
            for (let i = 0; i < formLength; i++) {
                const nameValue = data[i];

                if (nameValue.name.startsWith('o_fio_encrypt') && nameValue.value != null && nameValue.value !== '') {
                var encryptedText = encryptData(publicKey, nameValue.value);
                data2[nameValue.name] = encryptedText;
                } else {
                    data2[nameValue.name] = nameValue.value;
                }
            }

            // Wait for all encryption promises to complete
            Promise.all(pendingPromises)
                .then(() => resolve(data2)) // Resolve the main Promise with processed data
                .catch(reject); // In case of an error, reject the main Promise
        } catch (error) {
            reject(error); // Reject the main Promise in case of any other errors
        }
    });
}
	 ...
	 try {
        ...
        if (data && typeof data === 'object') {
            ...
            if (businessPath && isValidBusinessPath(businessPath)) {
                ...
                documentTitle = o_escapeHtml(documentTitle);
...
function o_ffXHREvent(formNam, dispIdField, dispId, eventIdField, eventInt, dirtyCheck, push, submit, busyCheck) {
	...
	if(submit) {
		...
		for(var i=0; i<formLength; i++) {
			...
			if(nameValue.name != 'dispatchuri' && nameValue.name != 'dispatchevent' && nameValue.name != 'o_fio_ignore') {
		...
		success: function(responseData, textStatus, jqXHR) {
			try {
				if (typeof responseData === 'object' && responseData !== null) {
                    if(push) {
                        try {
                            var businessPath = o_escapeHtml(responseData['businessPath']);
                            var documentTitle = o_escapeHtml(responseData['documentTitle']);
                            var historyPointId = o_escapeHtml(responseData['historyPointId']);
                            if(businessPath) {
                                o_pushState(historyPointId, documentTitle, businessPath);
                            }
                        } catch(e) {
                    ...
                    throw new Error('Unexpected responseData format');
                }
			} catch(e) {
				if(window.console) console.log(e);
			} finally {
...
function o_ffSetFocus(formId, formItemId) {
	...
	function isValidId(id) {
        return /^[a-zA-Z0-9_-]+$/.test(id); // Adjust the pattern as needed
    }

    var isInForm = function(formElementId, elementId) {
        if (!isValidId(formElementId) || !isValidId(elementId)) {
